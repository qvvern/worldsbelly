<template>
  <baseLayout class="recipe-prublish">
    <div slot="content" class="recipe-prublish__body" v-if="isPageReady">
        <v-toolbar
                class="recipe-prublish-toolbar"
                dense
                color="#080825"
                dark
                extended
        ...
        >
            ...
            <v-toolbar-title class="mx-auto" slot="extension">
                <span>{{ $t('YouHaveTotalOf') }} {{recipePublished.length}}</span>
                <h2>{{ $t('publishedRecipes') }}</h2>
        ...
        <template  v-if="recipePublished.length > 0">
          <div class="w-full flex flex-wrap p-3 justify-center" style="margin-top: -50px">
              <RecipeCard class="p-3" v-for="(draft, i) in recipePublished" :key="i" @click.prevent.native="goToRecipe(draft)" :value="draft" />
          </div>
        </template>
        <template v-else>
            ...
            <div class="drawing">
              ...
              <div class="flex w-full flex-col flex-wrap p-3 justify-center items-center">
                  <template v-if="recipeDrafts.length > 0">
                    <baseButton thick @click.prevent.native="$localeRouter.push({name: 'RecipeDraftsOverview'})">{{ $t('SeeYourDraftRecipes') }}</baseButton>
                    <h4 class="pb-2 my-2">{{$t('youHave')}} {{recipeDrafts.length}} {{recipeDrafts.length == 1 ? $t('recipe') : $t('recipes') }} {{$t('thatIsNotYetPublished')}}</h4>
    ...
    <template slot="loader">
        <baseLoading v-if="!isPageReady" :value="true" overlay parent fade :progressSentences="[`${$t('preparing')} ${$t('recipes')}`]" />
    </template>
  </baseLayout>
</template>
...
<style scoped>
    .recipe-prublish-toolbar {
        background: -webkit-linear-gradient(top, #d71d48, #8b0e0e 100%) !important;
